...
function formatTimeRemaingin(seconds: number) {
  const minites = Math.floor(seconds / 60);
  const secs = seconds % 60;
  return `${minites.toString().padStart(2, "0")}:${secs
    .toString()
    .padStart(2, "0")}`;
}
...
const Page = () => {
  ...
  const { data: messages, refetch } = useQuery({
    ...
    queryFn: async () => {
      const red = await client.api.message.get({ query: { roomId } });
      return red.data;
    },
  });
      ...
      <header className="border-b border-zinc-800 p-4 flex items-center justify-between bg-zinc-900/30">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            >
              {timeRemaining !== null
                ? formatTimeRemaingin(timeRemaining)
                : "--:--"}
            </span>
          </div>
        </div>
